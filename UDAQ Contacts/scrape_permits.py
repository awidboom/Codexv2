...
MAX_PERMITS = None  # set to an int for quick smoke test; None means full run
...
def find_pollutant(text, label):
    # Prefer per-line extraction: match label on a line and take the last numeric token on that line.
...
def parse_pdf(pdf_bytes, page_limit=12):
    ...
    if emissions_text:
        search_area = emissions_text
        if co2e == "":
            co2e = find_pollutant(search_area, r"CO\s*2\s*Equivalent")
        if co == "":
            co = find_pollutant(search_area, r"Carbon Monoxide")
        if nox == "":
            nox = find_pollutant(search_area, r"Nitrogen Oxides")
        if pm10 == "":
            pm10 = find_pollutant(search_area, r"PM\s*10")
        if pm25 == "":
            pm25 = find_pollutant(search_area, r"PM\s*2\.5")
        if so2 == "":
            so2 = find_pollutant(search_area, r"Sulfur Dioxide")
        if voc == "":
            voc = find_pollutant(search_area, r"Volatile Organic Compounds")
        if total_haps == "":
            total_haps = find_pollutant(search_area, r"Total HAPs")
    return {
...
def parse_pdf_v2(pdf_bytes, page_limit=12):
    reader = PdfReader(BytesIO(pdf_bytes))
    pages = reader.pages[:page_limit]
    text = "\n".join((p.extract_text() or "") for p in pages)
    ao_id = ""
