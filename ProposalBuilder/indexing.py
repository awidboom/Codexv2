...
def cache_meta_path(folder: str) -> str:
    return os.path.join(folder, ".rag_cache", "index_meta.json")
...
def write_cache_meta(folder: str, cache: Dict) -> None:
    meta = cache.get("meta", {}) if isinstance(cache, dict) else {}
    payload = {
        "cache_path": cache_path(folder),
        "version": cache.get("version") if isinstance(cache, dict) else None,
        "last_indexed": meta.get("last_indexed"),
        "last_updated": meta.get("last_updated"),
        "total_files_on_disk": meta.get("total_files_on_disk", meta.get("total_files")),
        "total_files_in_index": meta.get("total_files_in_index", meta.get("total_files")),
        "new_files_detected": meta.get("new_files_detected"),
        "failed_files": meta.get("failed_files") or [],
        "missing_source_files": meta.get("missing_source_files") or [],
    }
    path = cache_meta_path(folder)
...
def load_cache(folder: str) -> Dict[str, Dict[str, str]]:
    ...
    try:
        ...
        if isinstance(data, dict) and "entries" in data:
            try:
                if not os.path.exists(cache_meta_path(folder)):
                    write_cache_meta(folder, data)
            except Exception:
                pass
            return data
    except Exception:
...
def save_cache(folder: str, cache: Dict[str, Dict[str, str]]) -> None:
    ...
    if isinstance(cache, dict):
        ...
        cache["meta"]["last_updated"] = _now_iso()
    with open(path, "wb") as f:
        pickle.dump(cache, f)
    try:
        write_cache_meta(folder, cache)
    except Exception:
        pass
...
def load_documents_cached(
    ...
    missing_in_fs = sorted(list(cached_keys - current_keys))

    # Fast path: no on-disk changes and not forcing; trust cached index.
    # If allow_missing_sources is set, also keep cached entries for files no longer present.
    if (
        (not force_reindex)
        and entries
        and (current_keys == cached_keys or (allow_missing_sources and current_keys.issubset(cached_keys)))
        and new_files_count == 0
    ):
        docs: List[DocEntry] = []
        for rel, path in files.items():
        ...
        if allow_missing_sources and missing_in_fs:
            for rel in missing_in_fs:
                cached = entries.get(rel, {})
                docs.append(
                    DocEntry(
        ...
        cache["meta"] = {
            ...
            "total_files_in_index": len(entries),
            "new_files_detected": 0,
            "failed_files": meta_in.get("failed_files", []) or [],
            "missing_source_files": missing_in_fs if allow_missing_sources else [],
    ...
    if allow_missing_sources and entries:
        preserved = cached_keys - current_keys
        if preserved:
            for rel in preserved:
                if rel not in new_entries:
                    new_entries[rel] = entries.get(rel, {})
                    cached = entries.get(rel, {})
                    docs.append(
                        DocEntry(
                            path=os.path.join(folder, rel),
            ...
            meta["missing_source_files"] = sorted(list(preserved))
        else:
            meta["missing_source_files"] = []
    else:
        meta["missing_source_files"] = []

    meta["total_files_in_index"] = len(new_entries)
    out_cache = {"version": 1, "entries": new_entries, "meta": meta}
    save_cache(folder, out_cache)
    return docs, out_cache
